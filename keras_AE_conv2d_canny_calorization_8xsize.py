...
def plot_fig_vae2(x_test,x_test_gray, vae_imgs,k):
    n = 10
    plt.figure(figsize=(16, 16))
    for j in range(0,n):
        for i in range(1,n+1):
            ax = plt.subplot(10, n*1, i + 10*j)
            plt.imshow(vae_imgs[i+10*j].reshape(256,256,3))    #128, 128,3))
            #plt.gray()
            ax.get_xaxis().set_visible(False)
            ax.get_yaxis().set_visible(False)
...
vae.compile(loss='mse',optimizer='adam',metrics=['accuracy']) #loss='binary_crossentropy' #loss="mse"
...
imgs=[]
for i in range(len(x_train1)):
    x_train_gray =  rgb2gray(x_train1[i]).reshape(32,32)
    x_train_gray = img_to_array(x_train_gray*255).astype(np.uint8)
    x_train_canny = cv2.Canny(x_train_gray, 128,128 )
    x_train_canny = cv2.bitwise_not(x_train_canny)
    #x_train_canny = img_to_array(x_train_gray[i])
    imgs.append(x_train_canny)
x_train_gray=img_to_array(imgs).reshape(len(x_train1),32,32,1)
imgs2=[]
for i in range(len(x_test1)):
    x_test_gray =  rgb2gray(x_test1[i]).reshape(32,32)
    x_test_gray = img_to_array(x_test_gray*255).astype(np.uint8)
    x_test_canny = cv2.Canny(x_test_gray, 128,128 )
    x_test_canny = cv2.bitwise_not(x_test_canny)
    imgs2.append(x_test_canny)
x_test_gray=img_to_array(imgs2).reshape(len(x_test1),32,32,1)

#vae.load_weights('./size/ae_cifar10_weights_color_size14.h5',by_name=True) 
...
class Check_layer(keras.callbacks.Callback): 
    def on_epoch_end(self, epoch, logs={}):
        ...
        if s%20==0:
            ...
            plot_fig_vae(x_test1,x_test_gray,vae_imgs,k=s)
            plot_fig_vae2(x_test1,x_test_gray,vae_imgs,k=s)
            plot_fig4(x_test1,x_test_gray,vae_imgs,k=s)
