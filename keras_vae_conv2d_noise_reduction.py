...
def plot_fig_vae(x_test_noisy, vae_imgs,j):
    n = 100
    ...
    for i in range(1,n):
        ...
        plt.imshow(vae_imgs[i].reshape(28, 28))
        plt.gray()
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
    plt.savefig("mnist_noisy_vae_{}".format(j))    
    plt.pause(1)
    plt.close()
...
def plot_results(models,
    ...
    filename2 = "digits_over_latent_noisy"
...
plot_fig_vae(x_test_noisy,vae_imgs,0)
...
for i in range(n):
    # original_image
    orig_img0 = x_test[i].reshape(image_size, image_size)
    orig_img = x_test_noisy[i].reshape(image_size, image_size)
    ...
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
plt.savefig("autodetect_noise_reduction.jpg")
plt.pause(1)
plt.close()
...
vae.save_weights('ae_mnist_weights_nr.h5')
