...
model.load_weights("./category/cifar10/cat_mnist_weights_"+str(10)+".h5")  #s
...
n=20
for i in range(n):
    img=x_test1[int(np.random.randint(10000))]
    cat=model.predict(img.reshape(1,32, 32,3 ))  #28,28,1))
    #s=np.argmax(cat)
    s=0
    img2=encoder_decoders[s].predict(img.reshape(1,32, 32,3 )) #28,28,1))/255
    ax=fig.add_subplot(3,n,i+1)
    ax.set_title(str(s),size=40)
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    img=cv2.resize(img.reshape(32,32,3), size,interpolation = cv2.INTER_CUBIC) 
    ...
    plt.imshow(np.clip(diff_img,0,1), cmap=plt.cm.jet)
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
plt.savefig("./category/cifar10/autodetect_cifar20_rand_s0.jpg")
