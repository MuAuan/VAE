...
def plot_fig_vae(x_test,x_test_noisy, vae_imgs,k):
    ...
    for j in range(0,n):
        for i in range(1,2*n+1):
            ...
            plt.imshow(vae_imgs[i+10*j].reshape(32,32,3)) #128, 128,3))
            #plt.gray()
            ax.get_xaxis().set_visible(False)
            ax.get_yaxis().set_visible(False)
    plt.savefig("./mnist1000/gray/"+str(10)+"/cifar10_color_{}.jpg".format(k))    
    plt.pause(1)
    plt.close()    
...
def plot_results(models,
    ...
    filename1 = "./mnist1000/gray/"+str(10)+"/vae_mean_noisy"+str(epochs)
...
def plot_irregular(x_test,x_test_gray,decoded_imgs,epoch):
    ...
    for i in range(n):
        ...
        # reconstructed_image
        reconst_img = decoded_imgs[i].reshape(32, 32,3)
        ...
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
    plt.savefig("./mnist1000/gray/10/autodetect_color"+str(epoch)+".jpg")
    plt.pause(1)
    plt.close()
...
class Check_layer(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        if epoch%20==0:
            vae.save_weights("./mnist1000/gray/"+str(s)+"/vae_mnist_weightsL2_"+str(epoch)+"_gray.h5")
            encoder.save_weights("./mnist1000/gray/"+str(s)+"/encoder_mnist_weightsL2_"+str(epoch)+"_gray.h5")
            decoder.save_weights("./mnist1000/gray/"+str(s)+"/decoder_mnist_weightsL2_"+str(epoch)+"_gray.h5")
