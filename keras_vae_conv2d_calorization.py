...
def plot_fig_vae(x_test_gray, vae_imgs,j):
    n = 100
    ...
    for i in range(1,n):
        ...
        plt.imshow(vae_imgs[i].reshape(32, 32,3))
        plt.gray()
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
...
x_train_gray =  rgb2gray(x_train).reshape(50000,32,32,1)
x_test_gray =  rgb2gray(x_test).reshape(10000,32,32,1)
...
plot_fig_vae(x_test_gray,vae_imgs,0)
...
for i in range(n):
    # original_image
    orig_img0 = x_test[i].reshape(image_size, image_size,3)
    orig_img = x_test_gray[i].reshape(image_size, image_size)

    # reconstructed_image
    reconst_img = decoded_imgs[i].reshape(image_size, image_size,3)
